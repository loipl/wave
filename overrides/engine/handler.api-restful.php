...
// LOADING API AND GATHERING INPUT DATA
    ...
    // get www-command & id based on REST request method
    switch ($requestMethod) {
        case 'GET':
            if (is_null($objectId)) {
                $inputData['www-command'] = $controllerName . '-all';
            } else {
                $inputData['www-command'] = $controllerName . '-get';
                $inputData['id'] = $objectId;
            }
            break;
        case 'POST':
        ...
        case 'PUT':
        case 'PATCH':
            $inputData['www-command'] = $controllerName . '-edit';
            $inputData['id'] = $objectId;
            break;
        case 'DELETE':
            $inputData['www-command'] = $controllerName . '-delete';
            $inputData['id'] = $objectId;
            break;
        default :
            $inputData['www-command'] = $controllerName . '-all';
    ...
    // input REST parameters
    for ($i = $startResource; $i < count($urlComponents) - 1; $i += 2) {
        $inputData[$urlComponents[$i]] = $urlComponents[$i+1];
    }
